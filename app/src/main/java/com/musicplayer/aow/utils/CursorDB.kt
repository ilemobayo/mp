...
//        val songFile = File(realPath)
    ...
    fun getAlbumSongs(context: Context, album: String): Cursor{
        WHERE = (MediaStore.Audio.Media.ALBUM_ID + "=$album AND " + MediaStore.Audio.Media.SIZE + ">0" )
        return context.contentResolver.query(
                MEDIA_URI,
                PROJECTIONS,
                WHERE,
    ...
    fun deletMusic(cursor: Cursor, context: Context) {
        val realPath = cursor.getString(cursor.getColumnIndexOrThrow(MediaStore.Audio.Media.DATA))
